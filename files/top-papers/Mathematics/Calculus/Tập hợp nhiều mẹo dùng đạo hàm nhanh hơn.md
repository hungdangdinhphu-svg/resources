...
Bằng cách áp dụng (nhân nghịch đảo tương đương với phép chia) $G\_1 \\cdot \\frac{1}{G\_2} = G\_1 \\cdot G\_2^{-1}$, cấu trúc phân số $\\frac{u}{v}$ sẽ được chuẩn hóa thành:
...
$$(f \\cdot v)^{(n)} = u^{(n)} \\Longrightarrow \\sum\_{k=0}^n \\binom{n}{k} f^{(k)} v^{(n-k)} = u^{(n)}$$
...
$$f^{(n)} \\cdot v + \\sum\_{k=0}^{n-1} \\binom{n}{k} f^{(k)} v^{(n-k)} = u^{(n)}$$
...
$$f^{(n)} = \\frac{1}{v} \\left\[ u^{(n)} - \\sum\_{k=0}^{n-1} \\binom{n}{k} f^{(k)} v^{(n-k)} \\right]$$
...
$$\\log\_a x \\longrightarrow \\frac{1}{\\ln a} \\cdot \\ln x$$
